...
def get_rev_hybrid_feats(race_config, strat):
    # Features: 3 (Compound ADDITIVE) + 225 (Degradation MULTIPLICATIVE)
...
for idx, race in enumerate(valid_races):
    ...
    # Base multiplier > -10.0 (Additive offset can be negative)
    # Degradation multiplier >= 0.0
    bounds = [(None, None)] * 3 + [(0.0, None)] * 225
    
    res = linprog(c, A_ub=A_full, b_ub=b_full, bounds=bounds, method='highs')
    
    if not res.success:
        print(f"Broke at Race {idx + 1} with REVERSE HYBRID model and margin 0.0!")
        break
    else:
        print(f"Passed Race {idx + 1}")

if res.success:
    print("ALL RACES PASSED WITH REVERSE HYBRID MODEL!")
