...
valid_races = [r for r in races if r['race_config']['track_temp'] == 32]
...
if res.success:
    ...
    # We will print the diffs and see if there's a jump
    np.set_printoptions(threshold=np.inf)
    print("SOFT DIFF MAX, MIN (after lap 11):", np.max(s_diff[11:]), np.min(s_diff[11:]))
    print("MEDIUM DIFF MAX, MIN (after lap 21):", np.max(m_diff[21:]), np.min(m_diff[21:]))
    print("HARD DIFF MAX, MIN (after lap 31):", np.max(h_diff[31:]), np.min(h_diff[31:]))
    
    print("\nFULL SOFT DIFF:")
    print(np.array_repr(s_diff, max_line_width=100))
    print("\nFULL MEDIUM DIFF:")
    print(np.array_repr(m_diff, max_line_width=100))
    print("\nFULL HARD DIFF:")
    print(np.array_repr(h_diff, max_line_width=100))
