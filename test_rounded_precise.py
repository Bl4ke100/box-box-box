...
def evaluate(W):
    ...
    # Strictly positive means d1 took longer than d2, which violates d1 finishing ahead of d2
    # But F1 rounding might mean if v is like 0.0001, it's virtually 0 and maybe tied.
    # So a true violation is v > 0.0001
    violations = np.sum(v > 0.0001)
    
    # Exact ties
    ties = np.sum(np.abs(v) < 0.0001)
    
    return violations, ties

W_rounded = [0.0, 1.3, 2.2, 0.3, 0.1, 0.05, 0.0015, 0.00075, 0.0004]
v1, t1 = evaluate(W_rounded)
print(f"Rounded params violations: {v1}, exact ties: {t1}")
