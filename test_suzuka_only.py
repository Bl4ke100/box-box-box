...
for idx, race in enumerate(valid_races):
    ...
    for i in range(len(finishing) - 1):
        ...
        # Test Multiplicative form!
        f1 = get_feats_arr(config, strats[d1]) * blt
        f2 = get_feats_arr(config, strats[d2]) * blt
    ...
    if not res.success:
        print(f"Broke at Suzuka Race {idx + 1}!")
        break
    else:
        print(f"Passed Suzuka Race {idx + 1}")

if res.success:
    print("ALL SUZUKA RACES PASSED!")
