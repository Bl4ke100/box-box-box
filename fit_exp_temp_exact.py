...
def get_time(race_config, strat, w):
    ...
    def add_stint(tire, stint_len):
        ...
        # Exponential Temp effect: deg_rate = deg_base * exp((temp - 25) * temp_factor)
        deg_rate = deg_base * np.exp((temp - 25) * temp_factor)
...
def loss(W):
    ...
    for race in eval_races:
        ...
        for i in range(len(times) - 1):
            ...
            if diff > 0.001:  # Count all violations
                total_loss += (diff + 0.001) ** 2
...
# Count exact violations
v_count = 0
for race in eval_races:
    config = race['race_config']
    strats = {s['driver_id']: s for _, s in race['strategies'].items()}
    finishing = race['finishing_positions']
    times = [get_time(config, strats[d], res.x) for d in finishing]
    for i in range(len(times) - 1):
