...
def main():
    ...
    def loss(W):
        ...
        for race in eval_races:
            ...
            # Precompute temp multiplier
            # Assumes Temp Effect scales degradation
            ...
            for i in range(len(times) - 1):
                diff = times[i] - times[i+1] # We want finishing[i] to have LESS time than finishing[i+1] -> diff < 0
                if diff > -0.0001:
                    violations += 1
                    total_margin += diff
    ...
    bounds = [
        (0.8, 1.1), (0.8, 1.2), (0.8, 1.3), # cs, cm, ch (Relative to BLT)
        (0.00001, 0.05), (0.00001, 0.05), (0.00001, 0.05), # deg s, m, h
        (1.0, 15.0), (3.0, 25.0), (10.0, 40.0), # thresholds
        (-0.02, 0.05), # temp scale
        (1.0, 3.0) # power p
    ]

    print("Starting Comprehensive DE solver...")
