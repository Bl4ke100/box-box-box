...
def main():
    ...
    # Evaluate across varied temperatures to catch TempFactor
    ...
    def loss(W):
        ...
        for race in eval_races:
            ...
            # Temp offset relative to a baseline, e.g., 20C or 0C
            temp = config['track_temp']
            t_mult = 1.0 + tmp_scale * (temp - 30.0)
    ...
    bounds = [
        (-2.0, 0.0), (-0.5, 0.5), (0.0, 2.0), # cs, cm, ch (ADDITIVE!)
        (0.0001, 0.02), (0.0001, 0.02), (0.0001, 0.02), # deg s, m, h (multipliers of BLT)
        (0.0, 15.0), (0.0, 25.0), (0.0, 40.0), # thresholds
        (-0.05, 0.05), # temp scale
        (1.0, 2.0) # power p
    ]

    print("Starting exact Reverse Hybrid Equation DE solver...")
    res = differential_evolution(loss, bounds, maxiter=300, workers=1, popsize=20, disp=True)
