...
for race in races[:100]:
    ...
    for i in range(len(finishing) - 1):
        ...
        # Are they completely identical in my features?
        if all(abs(v) < 0.001 for v in diff_f) and abs(diff_c) < 0.001:
            s1 = strats[d1]
            s2 = strats[d2]
            
            # Are their raw strategies identical?
            if s1['starting_tire'] != s2['starting_tire'] or s1['pit_stops'] != s2['pit_stops']:
                print(f"BINGO! Different strategies but identical features!")
                print(f"D1 {d1}: {s1['starting_tire']} -> {s1['pit_stops']}")
                print(f"D2 {d2}: {s2['starting_tire']} -> {s2['pit_stops']}")
                exit(0)

print("No identical features with different raw strategies found in the first 100 races.")
