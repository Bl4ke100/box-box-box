...
def evaluate(args):
    ...
    for t, expected_rank_map in test_data:
        ...
        driver_times.sort(key=lambda x: (round(x[0], 5), round(x[1], 5), x[2]))
        
        for true_rank, (time, last_lap, grid, d_id) in enumerate(driver_times):
...
if __name__ == '__main__':
    cs_range = [round(x, 2) for x in [-2.0 + i*0.05 for i in range(41)]]
    ch_range = [round(x, 2) for x in [0.0 + i*0.05 for i in range(41)]]
    search_space = list(itertools.product(cs_range, ch_range))
